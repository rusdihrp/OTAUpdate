...
HTTPUpdateResult ESP32HTTPUpdate::updates(const String& url, uint32_t size, const String& currentVersion)
{
    HTTPClient http;
    http.begin(url);
    return handleUpdate(http, size, currentVersion, false);
}
 ...
 */
HTTPUpdateResult ESP32HTTPUpdate::handleUpdate(HTTPClient& http, uint32_t size, const String& currentVersion, bool spiffs)
{
        ...
        if(len == size) {
            ...
            if(!startUpdate) {
                ...
                if(spiffs) {
                    ...
                    Serial.println("[httpUpdate] runUpdate spiffs...\n");
                } else {
                    command = U_FLASH;
                    DEBUG_HTTP_UPDATE("[httpUpdate] runUpdate flash...\n");
                    Serial.println("[httpUpdate] runUpdate flash...\n");
                }
                ...
                if(runUpdate(*tcp, len, http.header("x-MD5"), command)) {
                    ret = HTTP_UPDATE_OK;
                    Serial.println("[httpUpdate] Update OK");
                    ...
                    ret = HTTP_UPDATE_FAILED;
                    Serial.println("[httpUpdate] Update failed");
